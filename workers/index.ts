	...
	async fetch(
		...
		if (
			...
			if (existing) {
				...
				await env.DB.prepare(
					...
					.bind(input.email, input.name || null, input.id)
					.run();
			...
			await env.DB.prepare(
				...
				.bind(input.id, input.email, input.name || null)
				.run();
		...
		if (
			...
			await env.DB.prepare(
				...
				.bind(input.id, userId, input.title || null)
				.run();

			const thread = await env.DB.prepare("SELECT * FROM threads WHERE id = ?")
				.bind(input.id)
				.first();
		...
		if (
			...
			const result = await env.DB.prepare(
				...
				.bind(input.title || null, threadId, userId)
				.run();
